...
#print(souptext)
    ...
    for a in soup.find_all("currentprice"):
        ...
        ebay_history = [{"ID":ID,"Description": Description,"Sold_Date":Sold_Date,"Sold_Price":Sold_Price} for ID, Description, Sold_Date, Sold_Price
        in zip(ID, Description, Sold_Date, Sold_Price)]

        with open(file_name, "w",  encoding="utf-8", newline="") as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=["ID","Description","Sold_Date","Sold_Price"])
            writer.writeheader()
